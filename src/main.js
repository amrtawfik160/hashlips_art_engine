...
const layersSetup = (layersOrder) => {
    const layers = layersOrder.map((layerObj, index) => ({
        ...
        rarity: layerObj?.options?.rarity || 100,
        name:
            layerObj.options?.['displayName'] != undefined
                ? layerObj.options?.['displayName']
                : layerObj.name,
        blend:
            layerObj.options?.['blend'] != undefined ? layerObj.options?.['blend'] : 'source-over',
...
const startCreating = async () => {
    ...
    while (layerConfigIndex < layerConfigurations.length) {
        ...
        while (editionCount <= layerConfigurations[layerConfigIndex].growEditionSizeTo) {
            // filter layers with no chance
            const filteredLayers = layers.filter(({ rarity }) => isHasChance(rarity))
            let newDna = createDna(filteredLayers)
            if (isDnaUnique(dnaList, newDna)) {
                let results = constructLayerToDna(newDna, filteredLayers)
                let loadedElements = []
